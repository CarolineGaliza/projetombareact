...
class MapaGeografico extends Component {
  ...
  getData() {
    if (!("objects" in this.state.dt)) {
      d3.json("/melhor_em_casa.json")
        .then((data) => {
  ...
  drawChart(state) {
    ...
    if ("objects" in dt && this.drawed === false) {
...
// ${isNaN(this.data.get(`${d.id.slice(0, 2)}-${d.properties.name}`)) ? 'Não disponível' : `${this.format(this.data.get(`${d.id.slice(0, 2)}-${d.properties.name}`))}%`}`);
      ...
      const gdadosm = this.svg.append("g").attr("id", "gdadosm");
      gdadosm.selectAll("path")
        .data(topojson.feature(dt, dt.objects.Melhor_em_Casa).features)
        .join("path")
            .attr("fill","red")
            .attr("stroke", "none")
      ...
      this.zoom = d3.zoom()
        .on("zoom", () => {
          ...
          gdadosm.attr("transform", d3.event.transform);
        });
